...
| 파라미터 | 현재값 | 설명 | 튜닝 팁 |
|----------|--------|------|---------|
| `x_offset` | -0.018 m | 발 기준 몸통 전후 오프셋 | 음수 = 몸통 뒤로. 앞으로 넘어지면 더 음수로 |
| `y_offset` | 0.000 m | 발 기준 몸통 좌우 오프셋 | 좌우 비대칭 시 조정. 좌우 대칭이면 0 |
| `z_offset` | 0.010 m | 발 기준 몸통 높이 오프셋 | 높이면 무릎 더 굽힘 |
| `hip_pitch_offset` | 1.0 deg | 고관절 전방 경사각 | 클수록 앞으로 기울어짐. 앞으로 넘어지면 줄임 |
...
| 파라미터 | 현재값 | 설명 | 튜닝 팁 |
|----------|--------|------|---------|
| `period_time` | 800 ms | 한 보행 사이클 시간 | 길수록 느리고 안정적, 짧으면 빠르지만 불안정 |
| `dsp_ratio` | 0.3 | 양발 지지 구간 비율 (0~1) | 클수록 안정적, 작을수록 빠름 |
